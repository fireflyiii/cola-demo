...
│   ├── security/                  # JWT 认证 + API Key 认证 + Spring Security
...
系统支持两种认证方式：**JWT Token**（内部用户登录）和 **API Key**（第三方系统调用）。
...
```bash
# 创建API应用（需要ADMIN角色）
curl -X POST http://localhost:8080/api-app/add \
  -H "Content-Type: application/json" -H "Authorization: Bearer {token}" \
  -d '{"appName":"第三方订单系统","allowedPaths":"/open/**"}'

# 响应中包含自动生成的API Key
# {"success":true,"data":{"id":1,"appName":"第三方订单系统","apiKey":"ak_xxxx...","allowedPaths":"/open/**","status":1}}
```
...
```bash
# 通过API Key访问Order接口
curl -X POST http://localhost:8080/open/order/add \
  -H "Content-Type: application/json" -H "X-API-Key: ak_xxxx..." \
  -d '{"orderName":"订单001","amount":100.00,"customerName":"张三"}'

curl "http://localhost:8080/open/order/list?orderName=订单" \
  -H "X-API-Key: ak_xxxx..."
```
...
订单接口位于 `/open/order` 路径下，支持 JWT Token 和 API Key 两种认证方式。
...
# 查询订单列表（按名称模糊查询）
curl "http://localhost:8080/open/order/list?orderName=订单" -H "X-API-Key: {apiKey}"
```
